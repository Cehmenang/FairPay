...
<div class="flex h-dvh w-full items-center justify-center bg-primary px-4 py-10">
  ...
  >
    ...
    <p bind:this={formEls[1]} class="mt-4 text-center text-sm text-third/70 sm:text-base">
      Log in to your FairPay account
    </p>
    ...
    <form onsubmit={handleSubmit} class="flex flex-col gap-4 mt-10">
      ...
      <div bind:this={formEls[5]} class="relative">
        <input
          bind:value={password}
    ...
    <p bind:this={formEls[7]} class="mt-6 text-center text-sm text-third sm:text-base">
      Don't have an account?
      <a href="/register" class="font-medium text-fourth hover:underline">Sign up</a>
    </p>
  </div>
</div>